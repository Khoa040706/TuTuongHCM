...
import {
  clearManualReview,
  setManualReview
} from "@/lib/server/learning-repository";
...
export async function PUT(request, { params }) {
  try {
    ...
    const data = await setManualReview(user.uid, {
...
export async function DELETE(request, { params }) {
  try {
    ...
    return jsonSuccess(await clearManualReview(user.uid, subjectId, subsectionId));
  } catch (error) {
    return routeError(error);
  }
}
