...
export async function gradeAndRecordQuiz(user, input) {
  ...
  const questionSetStatus = inspectQuestionSetIds(pool, input.questionsState);
  if (questionSetStatus === QUESTION_SET_STATUS.DUPLICATE_SUBMITTED_ID) {
    throw new ApiError(400, "DUPLICATE_QUESTION_ID", "Danh sách có câu hỏi bị lặp.");
  }
  if (questionSetStatus !== QUESTION_SET_STATUS.MATCH) {
    throw new ApiError(400, "QUESTION_SET_MISMATCH", "Bài làm không khớp toàn bộ đề đã cấp.");
  }
